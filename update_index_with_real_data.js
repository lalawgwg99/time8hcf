            ...
            if (matchedStores.length > 0 && (tab === 'all' || tab === 'stores')) {
                ...
                matchedStores.forEach(s => {
                    ...
                    if (sectionChief) {
                        ...
                        html += '<div style="display:flex; gap:6px;"><span style="color:var(--label-tertiary); width:36px;">課長</span><span>' + escapeHtml(sectionChief.split('\\n')[0]) + '</span></div>';
                        if (sectionPhone) {
                            html += '<a href="tel:' + cleanSecPhone + '" style="font-size:11px; font-weight:600; color:var(--blue); text-decoration:none; display:inline-flex; align-items:center; gap:3px;"><span>' + escapeHtml(sectionPhone) + '</span></a>';
                        }
                        html += '</div>';
                    }
                    if (assistant) {
                        html += '<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:2px;">';
                        html += '<div style="display:flex; gap:6px;"><span style="color:var(--label-tertiary); width:36px;">助理</span><span>' + escapeHtml(assistant.split('\\n')[0]) + '</span></div>';
                        if (assistantPhone) {
                            html += '<a href="tel:' + cleanAstPhone + '" style="font-size:11px; font-weight:600; color:var(--blue); text-decoration:none; display:inline-flex; align-items:center; gap:3px;"><span>' + escapeHtml(assistantPhone) + '</span></a>';
                        }
                        html += '</div>';
                    }
                    if (address) {
