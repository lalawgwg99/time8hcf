...
export async function onRequestPost({ request, env }) {
  ...
  if (Number(request.headers.get('Content-Length') || 0) > 20_000) return json({ error: 'Request payload is too large' }, 413, cors);
  ...
  try {
    ...
    const upstream = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'openrouter/free', messages: body.messages, max_tokens: 750, temperature: 0.7 }),
    });
